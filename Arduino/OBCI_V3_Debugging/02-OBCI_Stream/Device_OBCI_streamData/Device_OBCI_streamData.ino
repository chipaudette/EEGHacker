...
boolean packetTimeing = false;  
int bytesInPacket;

boolean packetTimeOut = false;
...
if(Serial.available()){
  if(streamingData){    // if we are streaming data
    ...
    if(packetTimeing && (micros() - packetTimer > 1000)){
      firstStreamingByte = true;  // delete the lossy packet
      packetTimeing = false;      // turn off the packetTimer
    }
    ...
    packetTimeOut = true;         // remember to check timer
...
void streamSerialBytes(){   // used in streamingData mode to receive serial bytes
  ...
  if(streamingByteCounter == bytesInPacket){ // when we get all the bytes
    ...
    serialBuffer[head][serialIndex[head]] = 0x01;  // place the 0x01 packetCounter
    serialIndex[head]++;                     // increment pointer
    streamingByteCounter = 0;                // initialize streamingByteCounter
    packetTimeOut = false;                   // turn off streamin idle timer
