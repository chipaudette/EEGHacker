...
switch 40
    ...
    case 30
        fname = '07-openBCI_raw_2014-04-30_22-41-40_countBackBy3.txt';nchan=2;
        s_fname = '2014-04-30 Evening, Count Back by 3';
        t_sig_sec = [74 128];
        %t_baseline_sec = [27*60+14.7 27*60+44.9];
        t_baseline_sec = [37 61];
        %t_plot_sec = [680 1890];
        notch60=1;notch70=1;notch120=1;
    case 40
        fname = 'openBCI_raw_2014-05-08_21-24-31_countbackby3_countbackby41from1200.txt';nchan=1;
        s_fname = '2014-05-08 Evening,Count Back by 3, Count Back by 41';
        t_sig_sec = [80 140];  %count back by three
        t_sig_sec = [306 398]; %count back by 41 from 1200
        %t_baseline_sec = [27*60+14.7 27*60+44.9];
        t_baseline_sec = [48 66];
        %t_plot_sec = [680 1890];
        notch60=1;notch70=0;notch120=1;        
end
...
for Iplot=1:nPlots
    ...
    for Idata=1:size(t_sig_sec,1);
                ...
                yl=ylim;
                if (Iplot==3)
                    for Ichan=1:size(std_data_V,2)
                        for Itype=1:2
                            switch Itype
                                case 1
                                    val = std_data_V(Idata,Ichan)*1e6;
                                    x = t_sig_sec(Idata,:);
                                case 2
                                    val = std_noise_data_V(Idata,Ichan)*1e6;
                                    x = t_baseline_sec(Idata,:);

                            end

                            hold on;plot(x,val*[1 1],'k:','linewidth',2);hold off
                            h=text(mean(x),val+0.05*diff(yl),{num2str(val,3); 'uVrms'});
