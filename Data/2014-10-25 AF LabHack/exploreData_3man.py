...
channels_to_plot = [5,6,7,8]  # counting the first channel as one
...
eeg_data_uV = data[:, 1:]   # ignore the first channel (column 0), so channel 1 is column 1
...
n_row = len(channels_to_plot)


for Iplot in range(len(channels_to_plot)):
    Ichan = channels_to_plot[Iplot]
    print "Processing " + str(Ichan) + ", which is " + str(Iplot) + " of " + str(len(channels_to_plot))     
    ...
    spec_PSDperHz, freqs, t = mlab.specgram(np.squeeze(f_eeg_data_uV[:,Ichan]),
                                   NFFT=NFFT,
    ...
    plt.title(fname + ', Chan ' + str(Ichan))
    if (len(t_lim_sec) == 0):
