...
int systemMode = 0; //used to switch between application states
/*
Modes:
0 = intro screen
10 = gui
...
final int eegDataSource = 0;

//Serial communications constants
OpenBCI_ADS1299 openBCI = new OpenBCI_ADS1299(); //dummy creation to get access to constants, create real one later
String openBCI_portName = "placeholder";   /************** CHANGE THIS TO MATCH THE COM PORT REPORTED ON *YOUR* COMPUTER *****************/
// String openBCI_portName = "/dev/tty.usbserial-A700fiI9"; 
...
void mousePressed() {
  ...
  if(controlPanel.isOpen){
    //if clicked in panel
    if(mouseX > 0 && mouseX < controlPanel.panelWidth && mouseY > 0 && mouseY < controlPanel.panelHeight){
      println("CP clicked");
